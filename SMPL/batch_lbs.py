...
def batch_skew(vec, batch_size=None):
    ...
    """
    with tf.compat.v1.variable_scope("batch_skew", [vec]):
...
def batch_rodrigues(theta, name=None):
    ...
    Theta is N x 3
    """
    with tf.compat.v1.variable_scope(name, "batch_rodrigues", [theta]):
...
def batch_lrotmin(theta, name=None):
    ...
    """
    with tf.compat.v1.variable_scope(name, "batch_lrotmin", [theta]):
        with tf.compat.v1.variable_scope("ignore_global"):
            theta = theta[:, 3:]
...
def batch_global_rigid_transformation(Rs, Js, parent, rotate_base=False):
    ...
    """
    with tf.compat.v1.variable_scope("batch_forward_kinematics", [Rs, Js]):
        ...
        def make_A(R, t, name=None):
            ...
            with tf.compat.v1.variable_scope(name, "Make_A", [R, t]):
